...
config_file_path='server.json'
#config_topology_path='ip.json'

#specific_key='electronic'

def __ip_address(specific_key, config_path):
    with open(config_path) as f:
       data = json.load(f)
       key = data[specific_key]
...
def send_message(data):
    ...
    print jsonData
    message = jsonData['Request Message']
    if message['Type'] == "New Protocol":
    	serverName = message['Server Name']
    	print serverName
    	client.connect(( __ip_address(serverName, config_file_path), 8888 ))
    	client.send(data)
...
def send_packet():
	...
	packet = '';
	reply = '';
	source_ip = __ip_address('self', config_topology_path)
	dest_ip = __ip_address('electronic', config_file_path)
	user_data = 'Hello, how are you'
	packet = generate_ip_header(source_ip, dest_ip) + generate_tcp_header(source_ip, dest_ip, user_data) + user_data
